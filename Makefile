...
init: check-prereqs setup-env install-python load-data test-system
	...
	@echo "   cd src/agents/Lumina && python3 Lumina_sql.py \"Ingenierías en Bogotá\""
...
load-data:
	...
	@mysql -h $(DB_HOST) -P $(DB_PORT) -u root -p < "$(SCHEMA_FILE)"
	@echo "   → Importando datos (~15MB comprimidos, 1-3 min)..."
	@gunzip -c "$(DUMP_FILE)" | mysql -h $(DB_HOST) -P $(DB_PORT) -u root -p $(DB_NAME)
...
install-python:
	...
	@cd "$(PROJECT_ROOT)/src/agents/Scrapper" && python3 -m playwright install chromium >/dev/null 2>&1 || true
...
web:
	...
	@cd "$(PROJECT_ROOT)/src/agents/Ada" && python3 web_ada.py
...
test-system test:
	...
	@cd "$(PROJECT_ROOT)/src/agents/Lumina" && python3 Lumina_sql.py "Cuenta programas de Ingeniería" 2>&1 | grep -q "tipo" \
...
clean:
	...
	@mysql -h $(DB_HOST) -P $(DB_PORT) -u root -p -e "DROP DATABASE IF EXISTS $(DB_NAME);"
